...
public class FormatoTabla extends DefaultTableCellRenderer {
    ...
    public Component getTableCellRendererComponent(JTable table, Object value, boolean selected, boolean focused, int row, int column) {
        ...
        table.setForeground(Color.darkGray);//color de texto
        ...
        if (String.valueOf(table.getValueAt(row, column)).trim().equals("") || String.valueOf(table.getValueAt(row, column)).trim().equals("<vacio>")) {
        ...
        /*--------*/
        if (String.valueOf(table.getValueAt(row, column)).equals("Cancelada")) {
            setBackground(Color.red);
            table.setFont(negrilla);
            setHorizontalAlignment(0);//centro
        }
        /*--------*/
        if (String.valueOf(table.getValueAt(row, column)).equals("Vigente")) {
            setBackground(Color.yellow);
            table.setFont(negrilla);
            setHorizontalAlignment(0);//centro
        }
        /*--------*/
        if (String.valueOf(table.getValueAt(row, column)).equals("Saldada")) {
